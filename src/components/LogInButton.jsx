...
export default function LogInButton({navClosed}) {
    ...
    return (
        <>
            ...
            </button> : <div className='profilePicWrapper'><button className={`profilePic ${userSettings ? "is-active" : ""}`} onClick={() => toggleUserSettings()}><img alt="Profile Pic" src={authUser.photoURL}></img></button><button className='logOutBtn' onClick={() => authLogOut()} title="Cerrar Sesión"><i class="bi bi-box-arrow-right"></i></button></div>
            }
        </>
    )
}
