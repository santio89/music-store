...
export default function ItemDetailContainer() {
    ...
    const [spotifyBadReq, setSpotifyBadReq] = useState(false);
    ...
    useEffect(() => {
                ...
                if (res.ok) {
                    res.json().then(
                        res => {
                            ...
                            res.price = Math.ceil(Math.abs(100 + (res.community.have / 400) * 1.56));
                                    ...
                                    response.json().then(response => {
                                        ...
                                        fetch(`https://api.spotify.com/v1/search?q=${res?.title}%20${res?.artists_sort}&type=album`, {
                                            ...
                                            if (res.ok) {
                                                ...
                                                }).catch(err => { console.log("error fetching spotify: " + err); setSpotifyBadReq(true); })
                                            } else {
                                                setSpotifyBadReq(true);
                                            }
                                        }).catch(err => { console.log("error fetching spotify: " + err); setSpotifyBadReq(true); }).finally(() => setSpotifyLoading(false));
                                    })
                                } else {
                                    setSpotifyBadReq(true);
                                }
        ...
        ).catch(err => { console.log("error fetching product: ", err) }).finally(() => setSpotifyLoading(false));

    }, [productId])

    return (

        <ItemDetail producto={producto} loading={loading} spotifyId={spotifyId} spotifyLoading={spotifyLoading} spotifyBadReq={spotifyBadReq} />
